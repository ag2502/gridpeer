...
def test_confidence_grows_with_history() -> None:
    """Confidence rises as observations accrue, capped at 1.0."""
    forecaster = NaiveForecaster(window=4)
    ts = datetime(2026, 9, 16, 8, 0)
    low = forecaster.forecast("h1", 1, ts, [1.0], [1.0])
    full = forecaster.forecast("h1", 2, ts, [1.0, 2.0, 3.0, 4.0], [1.0, 2.0, 3.0, 4.0])
